...
const PaymentsList = () => {
  ...
  useEffect(() => {
    async function fetchTransactions() {
      try {
        ...
        if (!res.ok) throw new Error(data.error || "Ошибка при загрузке транзакций")
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h1 className="text-3xl font-bold mb-6">Список транзакций за последние 6 месяцев</h1>
      ...
      {transactions.length > 0 ? (
        ...
        <p className="text-gray-600">Транзакций не найдено за указанный период.</p>
      )}
    </div>
  )
}

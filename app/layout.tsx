...
export default async function RootLayout({
  ...
  const categoriesData = queryClient.getQueryData<IGetAllCategories>( ['categories'] ) || { success: false, count: 0, categories: [] };
  const brandsData = queryClient.getQueryData<IGetAllBrands>(['brands']) || { success: false, count: 0, brands: []  };
  
  // Check if data exists and has content
  const hasValidData = 
    pricesData?.minPrice !== undefined && pricesData.maxPrice !== undefined &&
            ...
            {session?.user ? (
              ...
              hasValidData ? (
                <HydrationBoundary state={dehydrate(queryClient)}>
                  <Sidebar pricesData={pricesData} categoriesData={categoriesData} brandsData={brandsData} />
                  <div className='w-full'>{children}</div>
                </HydrationBoundary>
              ) : (

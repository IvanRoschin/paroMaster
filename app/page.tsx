...
const SLIDES_QUERY_KEY = (params: ISearchParams) => ["slides", JSON.stringify(params)]
const TESTIMONIALS_QUERY_KEY = (params: ISearchParams) => ["testimonials", JSON.stringify(params)]
...
export default async function Home({ searchParams }: { searchParams: Promise<ISearchParams> }) {
  ...
  const slidesData = queryClient.getQueryData<IGetAllSlides>(SLIDES_QUERY_KEY(params))
  ...
  const testimonialsData = testimonialsDataRaw
    ? {
        ...testimonialsDataRaw,
        testimonials: testimonialsDataRaw.testimonials.filter((t: ITestimonial) => t.isActive)
      }
    : null
        ...
        {testimonialsData && (
          <section>
            <div className="flex flex-col">
              <TestimonialsList testimonialsData={testimonialsData} title="Відгуки клієнтів" />
            </div>
          </section>
        )}

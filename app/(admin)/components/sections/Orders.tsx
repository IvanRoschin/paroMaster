...
import { deleteOrderAction, getAllOrdersAction } from '@/actions/orders';
...
export default function Orders() {
  ...
  const { mutate: deleteOrderById } = useDeleteData(
    async (id: string) => {
      ...
      return { success: true, message: 'Замовлення видалено' };
    },
        ...
        {data.orders?.map((order: IOrder, i: number) => (
          ...
          >
            ...
            <div className="text-sm text-gray-800 mb-2 space-y-1">
              <p>
                <strong>Ім’я:</strong> {order.customerSnapshot.user.name}
              </p>
              <p>
                <strong>Телефон:</strong> {order.customerSnapshot.user.phone}
              </p>
              <p>
            ...
            <ul className="text-xs text-gray-600 list-disc pl-5 mb-2">
              {order.orderedGoods.map((item, i) => {
                let title = 'Товар видалено'; // fallback, если нет объекта
                if (item.good) {
                  title =
                ...
                return (
                  <li
                    key={`${typeof item.good === 'string' ? item.good : item.good?._id || i}-${i}`}
                  >

...
const CheckoutClient: React.FC<CheckoutClient> = ({ user }) => {
  ...
  useEffect(() => {
    ...
    // 2️⃣ Если пользователь авторизован — заполняем его данные
  ...
  const handleSubmit = async (values: ICustomerFormValues) => {
    ...
    try {
      ...
      if (user?.id) {
        ...
        if (userRes.message === 'Користувач вже існує') {
          // Просто продолжаем с существующим пользователем
          if (!userRes.user?._id)
            throw new Error('Не вдалося отримати існуючого користувача');
